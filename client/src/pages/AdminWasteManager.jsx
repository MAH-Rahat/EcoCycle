...
import { Package, Trash2, ArrowLeft, Loader2, Clock, Camera, MapPin, User, CheckCircle, XCircle } from 'lucide-react';
...
export default function AdminWasteManager() {
    ...
    const handleFetchWaste = async () => {
        ...
        try {
            ...
            // This error is now clearer on the backend
    ...
    const handleAction = async (id, action) => {
        ...
        const confirmMsg = `Are you sure you want to ${action.toLowerCase()} this request?`;
        ...
        try {
            const payload = {
                status: newStatus,
                collectorId: newStatus === 'Accepted' ? adminId : undefined,
            };

            const res = await axios.put(`http://localhost:5000/api/waste/status/${id}`, payload);

            if (res.status === 200) {
                // Optimistically update the UI: remove item from the pending list
            ...
            alert(`Failed to perform action: ${err.response?.data?.message || 'Server error.'}`);
        }
    };
    ...
    const WasteCard = ({ item }) => (
        <div className="bg-white p-4 rounded-lg shadow border border-gray-100 flex flex-col space-y-3">
            <div className="flex justify-between items-center border-b pb-2">
                <span className={`text-sm font-bold px-3 py-1 rounded-full text-white ${item.material === 'Plastic' ? 'bg-blue-500' : item.material === 'Paper' ? 'bg-yellow-600' : item.material === 'Metal' ? 'bg-gray-500' : item.material === 'Glass' ? 'bg-green-500' : 'bg-purple-500'}`}>
                    {item.material}
                </span>
            ...
            <div className="flex space-x-4">
                ...
                <div className="flex-grow">
                    ...
                    <p className="text-sm text-gray-600 flex items-center space-x-1">
                        ...
                        {/* CRITICAL FIX: Use optional chaining to prevent crash if .populate fails */}
        ...
        <div className="min-h-screen bg-gray-100">
            ...
            <header className="bg-gray-800 shadow-xl">
                <div className="max-w-6xl mx-auto px-4 flex justify-between items-center h-16">
                    ...
                    <button onClick={() => navigate('/admin-panel')} className="flex items-center space-x-2 text-gray-400 hover:text-white transition">
            ...
            <main className="max-w-6xl mx-auto py-8 px-4">
                <div className="mb-6 p-4 bg-white rounded-xl shadow-lg border-l-4 border-indigo-600">
                    ...
                    <p className="text-sm text-gray-500">Filter and assign items for collection.</p>
                </div>
