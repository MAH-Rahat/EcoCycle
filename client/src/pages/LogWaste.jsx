...
export default function LogWaste() {
  ...
  const handleChange = (e) => {
    ...
    setFormData({
      ...formData,
      [name]: name === "estimatedWeight" ? parseFloat(value) : value,
    });
  };
  ...
  const handleLogWaste = async (e) => {
    ...
    if (formData.estimatedWeight < 0.1) {
    ...
    try {
      ...
      if (res.status !== 201 || !res.data?._id) {
        throw new Error("Waste log failed: invalid server response");
      }

      const wasteId = res.data._id;

      // 2) SCHEDULE PICKUP (marks pickupDetails.isRequested = true)
      // If you later add address input, replace the demo address.
      await axios.post("http://localhost:5000/api/pickup/schedule", {
        citizen: citizenId,
        wasteItem: wasteId,
        address: "Demo Address (Citizen Location)",
        scheduledDate: new Date().toISOString(),
        timeSlot: "10:00 AM - 12:00 PM",
      });

      // 3) AUTO-ASSIGN A COLLECTOR (demo-safe)
      // This assigns the first collector user found in DB to this waste.
      await axios.post(`http://localhost:5000/api/pickup/assign-to-first-collector/${wasteId}`);

      alert(
        `SUCCESS! ${res.data.weight} KG of ${res.data.material} logged and pickup requested.\nWaste ID: ${wasteId}`
      );

      // Navigate to home
      navigate("/home");
    } catch (error) {
      console.error("Log & Schedule Error:", error.response?.data || error);
      alert(
        `Failed: ${
          error.response?.data?.message || error.message || "Server error. Check server terminal."
        }`
      );
    } finally {
      setSubmitting(false);
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
      <header className="bg-white shadow-md sticky top-0 z-20">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 flex justify-between items-center h-16">
          ...
          <button
            ...
            className="flex items-center space-x-2 text-gray-600 hover:text-[#4CAF50] transition"
          >
      ...
      <main className="flex-grow flex justify-center items-start py-10 px-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-3xl border-t-4 border-[#4CAF50] animate-fadeIn">
          <div className="flex items-center space-x-3 mb-8 border-b border-gray-200 pb-4">
            <Trash2 className="h-7 w-7 text-[#4CAF50]" />
            <h2 className="text-2xl font-bold text-gray-800">Log New Recycling Item</h2>
          </div>
          ...
          <form onSubmit={handleLogWaste} className="space-y-8">
            <section>
              <h3 className="text-xl font-bold text-gray-700 mb-4 flex items-center">
                ...
                {formData.material && (
                  <span className="text-sm ml-3 px-3 py-1 bg-[#4CAF50]/10 text-[#4CAF50] font-bold rounded-full border border-[#4CAF50]/30 shadow-md">
                    {formData.material}
                  </span>
                )}
              </h3>
            ...
            <section className="pt-4 border-t border-gray-100">
              <h3 className="text-xl font-bold text-gray-700 mb-4">2. Provide Details & Photo Proof</h3>
            ...
            <div className="pt-6">
              <button
                type="submit"
                disabled={!formData.material || formData.estimatedWeight < 0.1 || submitting}
              ...
              >
                {submitting ? "Submitting..." : "Submit Item for Collection"}
              </button>
              <p className="text-sm text-center text-gray-600 mt-3">
                After submission, pickup is requested and assigned to a collector (demo flow).
              </p>
            </div>
          </form>
        </div>
